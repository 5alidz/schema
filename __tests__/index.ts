...
describe('composable schemas', () => {
  test('accepts other schemas as an object validator', () => {
    const schema = createSchema({ name: { type: 'string' }, age: { type: 'number' } });
    const containerSchema = createSchema({ children: { type: 'array', of: schema.getSchemaValidator() } });
    ...
    expect(
      ...
      { key: 'children[1].age', message: 'expected number but received undefined' },
      { key: 'children[2].name', message: 'expected string but received number' },
    ]);
  });
});
...
describe('array validator', () => {
  ...
  test('validates when required', () => {
    ...
    expect(validateArray(validator, ['hello', 42])).toStrictEqual(['[]1 expected string but received number']);
